...
def test_local_startup_docs_describe_managed_postgres_contract():
    ...
    assert "PostgreSQL CLI/server tools" in readme_text
    assert "make api" in readme_text
    assert "`make api`, `make dev`" in readme_text
    assert "If you explicitly export `DATABASE_URI`" in readme_text
    assert "The raw binary does not start the managed local PostgreSQL helper for you." in readme_text
    ...
    assert "PostgreSQL CLI/服务端工具" in readme_zh_text
    assert "make api" in readme_zh_text
    assert "（`make api`、`make dev`、`make serve`" in readme_zh_text
    assert "如果你在启动前已经显式设置了 `DATABASE_URI`" in readme_zh_text
    assert "裸跑二进制不会帮你启动这套托管本地 PostgreSQL" in readme_zh_text
