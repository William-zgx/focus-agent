...
def _sdk_types_text(root: Path) -> str:
    return "\n".join(path.read_text() for path in sorted((root / "src" / "types").glob("*.ts")))
...
def test_frontend_sdk_files_exist_and_export_core_surfaces():
    ...
    assert "resumePathForRunId" in client_text
    assert "streamEventRunId" in client_text
    assert "const requestPath = resumePath ?? path" in client_text
    assert "const requestBody = resumePath ? {} : body" in client_text
    ...
    assert "cancelThreadHarnessRuns" in client_text
    assert "/v2/runs/${encodeURIComponent(runId)}/cancel" in client_text
    assert "/v2/threads/${encodeURIComponent(threadId)}/runs/cancel" in client_text
