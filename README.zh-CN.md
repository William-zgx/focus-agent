...
# 或者改用 MOONSHOT_API_KEY / ANTHROPIC_API_KEY，并搭配相应模型覆盖。
make docker-rebuild
...
- Provider 的密钥和 Base URL 默认直接来自 `/data/local.env`，但这个默认文件里仍然带着 `OPENAI_API_KEY=replace-me-locally`、`MOONSHOT_API_KEY=replace-me-locally` 这类占位值。发起第一条真实请求前，请先编辑 `/data/local.env`（或对应 volume 内的文件），或者在 `docker compose up` 前从宿主 shell 导出一次性的透传变量，例如 `ANTHROPIC_API_KEY`、`OPENAI_API_KEY`、`OPENAI_BASE_URL`、`MOONSHOT_API_KEY`、`MOONSHOT_BASE_URL`、`OLLAMA_API_KEY`、`OLLAMA_BASE_URL`、`TAVILY_API_KEY`。
