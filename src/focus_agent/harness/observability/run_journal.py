...
class InMemoryRunJournal:
        ...
        *,
        ...
        async with self._lock:
            entry = JournalEvent(
                ...
                event=event,
                data=payload,
                sequence=sequence if sequence is not None else _next_sequence(self._events.get(run_id, [])),
    ...
    async def list_events(
        ...
        *,
        ...
        async with self._lock:
            events = [entry for entry in self._events.get(run_id, []) if event is None or entry.event == event]
...
class SQLiteRunJournal:
    ...
    def _setup(self) -> None:
        with self._connect() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_run_events_run ON run_events(run_id, sequence)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_tool_events_run ON tool_events(run_id, sequence)")
            conn.commit()
