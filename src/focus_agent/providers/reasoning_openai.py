...
def _normalized_reasoning_content(content: Any) -> Any:
...
def _extract_reasoning_content_from_message(message: AIMessage) -> str | None:
    ...
    content = _normalized_reasoning_content(getattr(message, "content", None))
    if isinstance(content, list):
        ...
        for block in content:
            ...
            if block_type not in _REASONING_BLOCK_TYPES:
                continue
...
def _convert_reasoning_message_to_dict(message: Any) -> dict[str, Any]:
    ...
    if isinstance(converted, AIMessage):
        converted = AIMessage(
            content=_normalized_reasoning_content(getattr(converted, "content", None)),
...
class ReasoningAwareChatOpenAI(ChatOpenAI):
    """ChatOpenAI variant that preserves OpenAI-compatible reasoning_content."""
...
__all__ = ["ReasoningAwareChatOpenAI"]
