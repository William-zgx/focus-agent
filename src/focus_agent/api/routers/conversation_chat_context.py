...
def list_conversations(
    ...
    token_usage_by_root = {
        item.root_thread_id: _token_usage_for_root_thread(
            runtime=runtime, root_thread_id=item.root_thread_id
        )
    ...
    return ConversationListResponse(
        conversations=[
            _conversation_response(
                item.model_copy(
                    update={"token_usage": token_usage_by_root.get(item.root_thread_id, {})}
                )
            )
            for item in conversations
        ],
...
def create_conversation(
    ...
    return _conversation_response(
        record.model_copy(update={"token_usage": normalize_token_usage()})
    )
...
def update_conversation(
    ...
    return _conversation_response(
        record.model_copy(
            update={
                "token_usage": _token_usage_for_root_thread(
...
@router.post(
    "/v1/conversations/{root_thread_id:path}/archive", response_model=ConversationSummaryResponse
)
...
@router.post(
    "/v1/conversations/{root_thread_id:path}/activate", response_model=ConversationSummaryResponse
)
...
@router.post(
    "/v1/threads/{thread_id:path}/branch-actions/{action_id}/execute",
    response_model=BranchActionExecuteResponse,
)
...
@router.post(
    "/v1/threads/{thread_id:path}/branch-actions/{action_id}/dismiss",
...
@router.post(
    "/v1/threads/{thread_id:path}/context/preview", response_model=ThreadContextPreviewResponse
)
...
@router.post(
    "/v1/threads/{thread_id:path}/context/compact", response_model=ThreadContextCompactResponse
)
