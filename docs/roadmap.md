...
- `frontend-sdk` 已覆盖 conversation、branch tree、branch action、merge review、agent role routing 等核心 typed client 能力
...
- Agent 主路径已具备评测框架、Plan-Act-Reflect、记忆读写闭环、上下文预算一期、工具运行时并行/缓存/降级基础、Postgres trajectory 写入、request/trace correlation，以及按职责拆分的 Web observability overview / trajectory workbench
...
3. **生产化治理**
   ...
   - Audit 继续完善；readiness / metrics 运行态接口已补齐第一版，并已形成基于 `/metrics` 的告警建议口径
4. **回归与发布**
   - UI smoke、API smoke、eval 回归样本继续扩充；observability 发布门禁需要覆盖浏览器 `--scenario all`、trajectory failure promotion preview 和批量 replay
   - 发布文档、迁移文档、运维清单保持可执行
...
| Memory | 读写闭环与 Memory Curator 分支提升保护已接入 | `src/focus_agent/memory/` `/app/agent/governance` | 继续做质量评分和人工 review 队列 |
| Context Engineering | v2 已接入长上下文压缩决策、artifact refs、角色上下文视图与治理台预览 | `src/focus_agent/core/context_policy.py` `src/focus_agent/agent_context_engineering.py` `/app/agent/governance` | tokenizer 精算、真实语义摘要质量评测、artifact 生命周期治理 |
| Tool Runtime | 并行/缓存/降级基础已落地 | `src/focus_agent/capabilities/tool_runtime.py` | 参数校验、取消/超时策略、side-effect 串行边界 |
| Eval / Regression | 已有 `tests/eval/` 基线，支持 baseline 对比和 trajectory replay/promotion 数据集生成 | `tests/eval/` | 扩 golden cases、补失败 trajectory 回放样本、把更多能力接入 CI 门禁 |
| Observability | trajectory 写入、request/trace correlation、查询/导出 CLI、replay/promotion 闭环、`/readyz`、`/metrics`、issue-discovery overview route、三栏 trajectory workbench、`timeline` / `zero_step` / `missing_detail` 证据态，以及浏览器 smoke 发布口径已落地 | `src/focus_agent/observability/trajectory.py` `src/focus_agent/observability/tracing.py` `apps/web/src/pages/observability/trajectory-page.tsx` | OpenTelemetry span/exporter 收尾、告警落盘、对比/批量治理 |
| Model Routing / Delegation | role routing、Memory Curator、Tool Router、Delegation Runtime、Model Router、Self Repair、Review Queue 的 API/SDK/Web 控制台与 eval gate 已补 | [agent-role-routing.md](agent-role-routing.md) `tests/eval/datasets/agent_delegation.jsonl` `/app/agent/governance` | 继续提升真实子任务执行质量、成本画像和人工 review 队列体验 |
| Autonomy | 治理底座已具备 | `/app/agent/governance` | 技能自选、分支建议、风险感知式工作流 |
...
- observability 治理体验：overview 与 trajectory 的职责已拆开，继续把告警接入、对比/批量治理和失败样本回放沉到常规发布流程
...
- observability 告警规则落地与更完整的批量治理 UI
- 技能自选与分支自主建议
...
1. 在保持现有 Postgres 主读写路径不变的前提下，把 `/metrics` 告警规则、trajectory failure promotion preview 和批量 replay 固化进发布流程，让 observability 从“能分诊、能复盘”进一步走向“可告警、可回归、可批量治理”。
2. 继续把受控 delegation 从治理记录推进到更强的子任务产物合成，优先解决 orchestrator / planner / executor / critic 的成本与质量分工，并继续保持 memory curator / skill scout 的隔离边界。
