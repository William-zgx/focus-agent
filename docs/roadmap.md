...
| Observability | trajectory 写入、request/trace correlation、查询/导出 CLI、replay/promotion 闭环、`/readyz`、`/metrics`、issue-discovery overview route、三栏 trajectory workbench，以及 `timeline` / `zero_step` / `missing_detail` 证据态已落地 | `src/focus_agent/observability/trajectory.py` `src/focus_agent/observability/tracing.py` `apps/web/src/pages/observability/trajectory-page.tsx` | 真实浏览器链路验证、OpenTelemetry span/exporter 收尾、对比/批量治理 |
...
- observability 治理体验：overview 与 trajectory 的职责已拆开，继续补更长时真实浏览器链路验证、对比/批量治理和告警接入
...
1. 在保持现有 Postgres 主读写路径不变的前提下，继续补真实浏览器链路验证、复盘批量治理和告警接入，让 observability 从“能分诊、能复盘”进一步走向“可对比、可告警、可回归”。
