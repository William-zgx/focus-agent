...
- Postgres 运维链：迁移验证报告已能阻断 release-health，继续补真实备份/恢复演练和长期保留策略
- observability 治理体验：告警报告已能阻断发布，继续把真实告警系统和长时浏览器回归接入 nightly
- Auth / Access Model：生产安全启动基线已强制检查 `AUTH_ENABLED`、JWT secret、demo token 与 rate limit，后续继续完善外部登录、令牌分发和访问模型
...
- 真实 CI provider 绑定：GitHub Actions / Buildkite artifact retention、approval job 和 release evidence 上传
- nightly regression ops：memory/context trend、trajectory replay、alert report、eval report 的长期趋势汇总
- production E2E / load smoke：真实部署下的 Web/SDK/API/graph smoke、安全 smoke、rate-limit 轻量压测

## 4. 下一阶段重点

未来一段时间建议优先做下面四件事：

1. 把 release evidence 的通用 storage / approval 字段绑定到真实 CI 平台，并保留 artifact retention 示例。
2. 把 memory/context trend、trajectory replay、alert report、eval report 接到 nightly 长期报表，避免只在手动发布前才发现漂移。
3. 扩 memory / eval / UI smoke 的真实失败样本，确保已落地的 agent 基线不会回退。
4. 增加生产部署下的 Web/SDK/API/graph E2E smoke、安全 smoke 和 rate-limit 轻量压测。
