...
| Model Routing | role routing、Memory Curator、Tool Router API/SDK/Web 控制台与 eval gate 已补 | [agent-role-routing.md](agent-role-routing.md) `tests/eval/datasets/agent_arch.jsonl` `tests/eval/datasets/agent_governance.jsonl` `/app/agent/governance` | 从工具绑定治理进入受控 delegation、成本路由与 helper fallback 观测 |
...
2. 把模型角色路由从 dry-run 推进到受控 delegation，优先解决 orchestrator / planner / executor / critic 的成本与质量分工，并继续保持 memory curator / skill scout 的隔离边界。
