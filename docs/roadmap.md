...
| Observability | trajectory 写入已落地 | 查询/导出 CLI、浏览器链路验证、OpenTelemetry 收尾 |
...
- Postgres 运维链：查询、导出、迁移验证、长期运维策略
...
1. 把 Postgres trajectory 的查询/导出链路补齐，让 observability 不只“能写”，也能真正用于复盘。
