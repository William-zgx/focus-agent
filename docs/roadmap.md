...
- Agent 主路径已具备评测框架、Plan-Act-Reflect、记忆读写闭环、上下文预算一期、工具运行时并行/缓存/降级基础、Postgres trajectory 写入与 Web review console
...
| Observability | trajectory 写入、查询/导出 CLI、replay/promotion 闭环和 Web review console 已落地 | `src/focus_agent/observability/trajectory.py` `apps/web/src/pages/observability/trajectory-page.tsx` | 真实浏览器链路验证、OpenTelemetry 收尾、生产化复盘视图 |
...
- trajectory 复盘体验：已有 `/app/observability/trajectory` review console，继续补真实浏览器链路验证、生产运营视图、OpenTelemetry tracing
...
1. 在保持现有 Postgres 主读写路径不变的前提下，继续补真实浏览器链路验证、生产运营视图和 OTel tracing，让 observability 从“能复盘”进一步走向“好复盘”。
