...
- Agent 主路径已具备评测框架、Plan-Act-Reflect、记忆读写闭环、上下文预算与 Context Engineering v2、工具运行时并行/缓存/降级基础、role/memory/tool/delegation/task-ledger 治理、Postgres trajectory 写入、request/trace correlation，以及按职责拆分的 Web observability overview / trajectory workbench
...
| Context Engineering | v2 已接入长上下文压缩决策、artifact refs、角色上下文视图与治理台预览；当前线程 `context_usage` 与非破坏式 compaction 已进入 ChatService / Web composer | `src/focus_agent/core/context_policy.py` `src/focus_agent/context_usage.py` `src/focus_agent/agent_context_engineering.py` `/app/agent/governance` `/app` | tokenizer 精算、真实语义摘要质量评测、artifact 生命周期治理、压缩质量可观测 |
...
| Observability | trajectory 写入、request/trace correlation、查询/导出 CLI、单条 replay/promotion、批量 promote-preview/replay-compare、`/readyz`、`/metrics`、overview route、三栏 trajectory workbench、`timeline` / `zero_step` / `missing_detail` 证据态，以及浏览器 smoke 发布口径已落地 | `src/focus_agent/observability/trajectory.py` `src/focus_agent/observability/tracing.py` `apps/web/src/pages/observability/trajectory-page.tsx` | OpenTelemetry 部署联通、告警落盘、长时浏览器回归 |
| Agent Governance | role routing、Memory Curator、Tool Router、Delegation Runtime、Model Router、Self Repair、Review Queue、Task Ledger 与 Delegated Artifact Synthesis 的 API/SDK/Web 控制台与 eval gate 已补 | [agent-role-routing.md](agent-role-routing.md) `tests/eval/datasets/agent_delegation.jsonl` `tests/eval/datasets/agent_task_ledger.jsonl` `/app/agent/governance` | 继续提升真实子任务执行质量、成本画像、critic gate 质量和人工 review 队列体验 |
...
- observability 治理体验：overview 与 trajectory 的职责已拆开，继续把告警接入和失败样本回放沉到常规发布流程
- Auth / Access Model：生产安全启动基线已强制检查 `AUTH_ENABLED`、JWT secret、demo token 与 rate limit，后续继续完善登录、令牌分发和访问模型
...
- observability 告警规则落地与更长时真实浏览器回归
- 技能自选与分支自主建议
...
1. 在保持现有 Postgres 主读写路径不变的前提下，把安全配置 smoke、`/metrics` 告警规则、trajectory failure promotion preview 和批量 replay 固化进发布流程，让 observability 从“能分诊、能复盘、能批量治理”进一步走向“可告警、可长期回归”。
2. 继续把受控 delegation 的 task ledger 和 delegated artifacts 用到更多真实执行链路里，优先解决 orchestrator / planner / executor / critic 的成本与质量分工，并继续保持 memory curator / skill scout 的隔离边界。
