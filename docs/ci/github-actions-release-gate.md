...
golden multi-agent, and memory/context reports. The nightly regression workflow
runs `model_matrix` and `trajectory_failures` as non-blocking reports under
`reports/nightly/`; upload those artifacts for trend review, but do not wire
them into production release blocking until their cases are promoted.
