...
export function useShellPreferences() {
  ...
  const selectedLanguage = LANGUAGE_OPTIONS.find((option) => option.value === languagePreference)
    ?? LANGUAGE_OPTIONS[0];
  const selectedTheme = THEME_OPTIONS.find((option) => option.value === themePreference)
    ?? THEME_OPTIONS[0];
  const selectedColor = COLOR_OPTIONS.find((option) => option.value === colorPreference)
    ?? COLOR_OPTIONS[0];
