...
export function useCreateAdminUser() {
  ...
  return useMutation<FocusAgentUser, Error, FocusAgentCreateUserRequest>({
    ...
    onSuccess: (user) => {
      ...
      void queryClient.invalidateQueries({ queryKey: queryKeys.adminUser(user.user_id) });
      void queryClient.invalidateQueries({ queryKey: queryKeys.adminAuditEventsRoot });
    },
  });
}
...
export function useRevokeAdminUserSession(userId: string | null) {
  ...
  return useMutation<FocusAgentSession, Error, FocusAgentRevokeUserSessionRequest>({
    mutationFn: (request) => {
      if (!userId) throw new Error("Missing user id.");
      return client.revokeUserSession(userId, request);
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: userId ? queryKeys.adminUserSessions(userId) : queryKeys.adminUsersRoot });
      void queryClient.invalidateQueries({ queryKey: queryKeys.adminAuditEventsRoot });
    },
  });
}
...
export function useResetAdminUserPassword(userId: string | null) {
  ...
  return useMutation<FocusAgentUser, Error, FocusAgentAdminResetPasswordRequest>({
    mutationFn: (request) => {
      if (!userId) throw new Error("Missing user id.");
...
export function useAdminAuditEvents(request: FocusAgentAuditEventListRequest = {}) {
  const { client, ready, isAdmin } = useFocusAgent();
  const filtersKey = JSON.stringify(request);
