...
export function providerLogoLetter(
	provider: string,
	providerLogoLetter?: string | null,
) {
	const configuredLetter = String(providerLogoLetter || "")
		.trim()
		.slice(0, 1);
	return (
...
export function contextUsagePercent(usage?: ContextUsageResponse | null) {
	return Math.max(
		0,
		Math.min(100, Math.round(Number(usage?.used_ratio ?? 0) * 100)),
	);
}

export function contextUsageRemainingPercent(
	usage?: ContextUsageResponse | null,
) {
	return Math.max(0, 100 - contextUsagePercent(usage));
}

export function shouldShowContextCompactAction(
	usage?: ContextUsageResponse | null,
) {
...
export function contextUsageTone(usage?: ContextUsageResponse | null) {
	...
	if (usage.status === "over" || Number(usage.used_ratio || 0) >= 0.92)
		return "is-over";
	if (usage.status === "hot" || Number(usage.used_ratio || 0) >= 0.85)
		return "is-hot";
	if (usage.status === "warm" || Number(usage.used_ratio || 0) >= 0.7)
		return "is-warm";
	return "is-ok";
}
...
function thinkingAvailableLabel(isChineseUi: boolean) {
	return isChineseUi
		? "支持思考，可手动切换"
		: "Thinking available, toggle manually";
}
...
function thinkingStatusText(mode: string, isChineseUi: boolean) {
	return mode === "enabled"
		? thinkingOnStatusLabel(isChineseUi)
		: thinkingOffStatusLabel(isChineseUi);
}
...
export function thinkingOptionMetaLabel(
	...
	return thinkingMode
		? thinkingStatusText(thinkingMode, isChineseUi)
