...
export function invalidateBranchActionNavigationSurfaces(
  ...
  void queryClient.invalidateQueries({ queryKey: queryKeys.branchTree(rootThreadId) });
  void queryClient.invalidateQueries({ queryKey: queryKeys.thread(sourceThreadId) });
  void queryClient.invalidateQueries({ queryKey: queryKeys.thread(targetThreadId) });
}
