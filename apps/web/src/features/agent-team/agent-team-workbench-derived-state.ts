...
export type AgentTeamWorkbenchFinalResultStateKind =
	| "missing"
	| "ready"
	| "placeholder"
	| "blocked"
...
export interface AgentTeamWorkbenchFinalPreviewState
	extends AgentTeamWorkbenchFinalResultState {
	hasBundle: boolean;
	canApprove: boolean;
...
export function deriveWorkbenchEvidenceRiskState({
	...
	const changedFiles = uniqueNonEmptyStrings(
		tasks.flatMap((task) => task.changed_files ?? []),
	);
	const outputArtifactIds = uniqueNonEmptyStrings(
		tasks.flatMap((task) => task.output_artifact_ids ?? []),
	);
	const evidenceItems = uniqueNonEmptyStrings([
		...(view?.evidence ?? []),
		...outputArtifactIds,
		...(view?.artifacts ?? []).map(
			(artifact) => artifact.summary ?? artifact.title ?? artifact.artifact_id,
		),
...
export function missionHeaderStateForWorkbench({
	...
	const sessionTitle =
		displayTitle || session?.session_id || "Agent Team Mission";
	return {
	...
	return {
		...
		risks: bundleRisks.length
			...
			: [
					isChineseUi
						? "暂未发现明确风险。"
						: "No explicit risks have been reported yet.",
				],
		nextActionLabel: nextActionLabelForBundle(activeBundle, isChineseUi),
	};
}

export function finalAnswerStatusForBundle(
...
function finalResultStateForWorkbench(
	...
	if (!activeBundle) {
		return {
			...
			help: isChineseUi
				? "任务完成后再生成最终结果。"
				: "Generate the final result after task outputs are ready.",
			deliverable: false,
			summary: isChineseUi
				? "Final Preview 尚未生成。"
				: "Final Preview has not been generated.",
			warnings: [],
		};
	}
	const explicitStatus = finalAnswerStatusForBundle(activeBundle);
	const isSimulated =
		explicitStatus === "placeholder" || isBundleSimulated(activeBundle);
	...
	const isDeliverable =
		...
		(explicitStatus === "ready" ||
			activeBundle.recommended_next_action === "merge");
...
function nextActionLabelForBundle(
	bundle: AgentTeamMergeBundle | null,
	isChineseUi: boolean,
) {
	const finalStatus = finalAnswerStatusForBundle(bundle);
	if (finalStatus === "placeholder")
		return isChineseUi ? "模拟执行，需真实运行" : "Simulated; run for real";
	if (finalStatus === "blocked")
		return isChineseUi ? "先补齐任务产出" : "Complete missing outputs first";
	if (finalStatus === "error")
		return isChineseUi ? "先处理生成错误" : "Resolve synthesis error first";
	...
	if (action === "request_changes")
		return isChineseUi ? "先处理修改项" : "Request changes first";
	if (action === "split_followup")
		return isChineseUi ? "拆分后续任务" : "Split follow-up tasks";
	if (action === "discard")
		return isChineseUi ? "放弃本次结果" : "Discard this result";
...
function isBundleSimulated(bundle: AgentTeamMergeBundle) {
	return [
		...
		Boolean(
			item &&
				/(?:^|\b)(fake delegated|delegated fake run|run-|artifact-)/i.test(
					item.trim(),
				),
		),
	);
