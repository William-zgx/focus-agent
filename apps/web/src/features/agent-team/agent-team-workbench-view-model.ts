...
import { buildPhaseGroups, buildPhaseMapItems } from "./agent-team-workbench-phase-state";
import type { AgentTeamMergeBundle, AgentTeamSession, AgentTeamSessionView } from "./types";
...
export function useAgentTeamWorkbenchViewModel({
  ...
  const { changedFiles, evidenceItems, riskItems } = deriveWorkbenchEvidenceRiskState({
    activeBundle,
    tasks,
    view,
  });
