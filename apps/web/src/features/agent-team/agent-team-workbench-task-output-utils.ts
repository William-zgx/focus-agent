...
const BUNDLE_RAW_RUN_TEXT_PATTERN =
	/(?:^|\b)(fake delegated|delegated fake run|run-|artifact-)/i;
...
export function outputsForTask(
...
export function isRawRunText(value: string, mode: RawRunTextMode = "task") {
	return (
		mode === "bundle" ? BUNDLE_RAW_RUN_TEXT_PATTERN : TASK_RAW_RUN_TEXT_PATTERN
	).test(value.trim());
}
