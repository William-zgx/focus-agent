...
import type { AgentTeamArtifact, AgentTeamSessionView, AgentTeamTask, AgentTeamTaskOutput } from "./types";
import { useRetryAgentTeamTask } from "./use-agent-team";

type AgentTeamCockpitViewModel = ReturnType<typeof useAgentTeamWorkbenchViewModel> & {
  primaryAction: ReturnType<typeof useAgentTeamWorkbenchViewModel>["primaryAction"] & {
    busy?: boolean;
    label: string;
  };
};
...
export function AgentTeamCockpit({ actions, inspector, session, viewModel }: AgentTeamCockpitProps) {
  const { isChineseUi } = useShellUi();
  const selectedTaskState = viewModel.selectedTask ? (viewModel.taskDisplayState[viewModel.selectedTask.task_id] ?? null) : null;
  const blockedTaskState = viewModel.taskDisplayStates.find((state) => state.kind === "failed" || state.kind === "needs_attention");
  const blockedTask = blockedTaskState ? viewModel.tasks.find((task) => task.task_id === blockedTaskState.taskId) ?? null : null;

  return (
    <div className="fa-agent-team-cockpit-shell fa-agent-team-simple-shell">
      <MissionHeader actions={actions} blockedTask={blockedTask} blockedTaskState={blockedTaskState ?? null} session={session} viewModel={viewModel} />
      {blockedTask ? <BlockedTaskGuide actions={actions} task={blockedTask} taskState={blockedTaskState ?? null} /> : <MissionSteps viewModel={viewModel} />}
      <main className="fa-agent-team-simple-main fa-agent-team-cockpit-grid" aria-label="Agent Team">
        <div className="fa-agent-team-simple-left">
          <ExecutionGraph actions={actions} viewModel={viewModel} />
          <TaskList actions={actions} blockedTaskId={blockedTask?.task_id ?? null} viewModel={viewModel} />
        </div>
        <div className="fa-agent-team-simple-right">
          <TaskDetail actions={actions} selectedTaskState={selectedTaskState} session={session} viewModel={viewModel} />
          <OutputsPanel actions={actions} session={session} viewModel={viewModel} />
...
function MissionHeader({
  ...
  blockedTaskState: AgentTeamCockpitViewModel["taskDisplayStates"][number] | null;
  session: AgentTeamSessionView;
  viewModel: AgentTeamCockpitViewModel;
}) {
  const { isChineseUi } = useShellUi();
  const header = viewModel.missionHeaderState;
  const primaryDisabled = Boolean(viewModel.primaryAction.disabledReason) || Boolean(viewModel.primaryAction.busy);
  const primaryClick = blockedTask ? () => actions.onSelectTask(blockedTask.task_id) : actions.onPrimaryAction;
  ...
  return (
    <section className={`fa-agent-team-simple-hero fa-agent-team-cockpit-mission-header is-${blockedTask ? "warning" : header.tone}`.trim()}>
      <div className="fa-agent-team-simple-hero-main">
        <span className="fa-agent-team-simple-kicker">Agent Team</span>
        <h1 {...tooltipProps(header.goal || session.session.goal)}>{header.title}</h1>
        <p {...tooltipProps(blockedTaskState?.help ?? header.stageHelp ?? viewModel.missionStage.help)}>
          {blockedTask
            ? isChineseUi
        ...
        <div className="fa-agent-team-simple-meta fa-agent-team-cockpit-mission-meta">
          ...
          <span>{total ? (isChineseUi ? `${done}/${total} 个任务完成` : `${done}/${total} tasks done`) : isChineseUi ? "还没有任务" : "No tasks yet"}</span>
        </div>
      ...
      <div className="fa-agent-team-simple-hero-action">
        <span>{blockedTask ? (isChineseUi ? "先处理这里" : "Handle this first") : isChineseUi ? "现在就点这里" : "Click here next"}</span>
        <button
        ...
        >
          {blockedTask ? (isChineseUi ? "先看卡住任务" : "Review blocked task") : viewModel.primaryAction.label}
        </button>
...
function BlockedTaskGuide({
  ...
  return (
    <section className="fa-agent-team-blocked-guide">
      ...
      <button className="fa-agent-team-cockpit-button is-primary" onClick={() => actions.onSelectTask(task.task_id)} type="button">
...
function MissionSteps({ viewModel }: { viewModel: AgentTeamCockpitViewModel }) {
  ...
  const total = viewModel.missionProgress.total;
  const done = viewModel.missionProgress.done;
  const active = viewModel.missionProgress.running + viewModel.missionProgress.queued;
  const hasResult = viewModel.finalPreviewState.hasBundle;
  const steps = [
    {
      label: isChineseUi ? "DAG" : "DAG",
      value: total ? (isChineseUi ? `${total} 个任务` : `${total} tasks`) : isChineseUi ? "未生成" : "Not planned",
      state: total ? "done" : "active",
    },
    {
      label: isChineseUi ? "运行" : "Run",
      value: active ? (isChineseUi ? "进行中" : "Running") : `${done}/${total || 0}`,
      state: active ? "active" : done && done < total ? "ready" : total && done >= total ? "done" : "idle",
    },
    {
      label: isChineseUi ? "交付" : "Deliver",
      value: hasResult ? (isChineseUi ? "可查看" : "Ready") : isChineseUi ? "等待" : "Pending",
      state: hasResult ? "done" : total && done >= total ? "ready" : "idle",
    },
  ];

  return (
    <section className="fa-agent-team-simple-steps" aria-label={isChineseUi ? "Mission 状态" : "Mission status"}>
      {steps.map((step) => (
        <article className={`fa-agent-team-simple-step is-${step.state}`} key={step.label}>
          <span>{step.label}</span>
          <div>
            <strong>{step.value}</strong>
            <small>{step.state === "active" ? (isChineseUi ? "当前" : "Current") : step.label}</small>
          </div>
        </article>
      ))}
    </section>
    ...
    <section className="fa-agent-team-simple-panel fa-agent-team-execution-graph-panel">
      <header className="fa-agent-team-simple-panel-header">
        <div>
          ...
          <strong>{isChineseUi ? "依赖与执行状态" : "Dependencies and status"}</strong>
        </div>
      </header>
      {viewModel.tasks.length ? (
        <div className="fa-agent-team-graph-canvas">
          <div className="fa-agent-team-graph-main" style={{ height: graph.height, width: graph.width }}>
            <svg className="fa-agent-team-graph-lines" height={graph.height} viewBox={`0 0 ${graph.width} ${graph.height}`} width={graph.width} aria-hidden="true">
              {graph.edges.map((edge) => {
                const from = graph.nodeById.get(edge.from);
                const to = graph.nodeById.get(edge.to);
                if (!from || !to) return null;
                return <path className="fa-agent-team-graph-edge" d={taskEdgePath(from, to)} key={`${edge.from}-${edge.to}`} />;
              })}
            </svg>
            {graph.nodes.map((node) => {
              const taskState = viewModel.taskDisplayState[node.task.task_id];
              const isSelected = viewModel.selectedTask?.task_id === node.task.task_id;
              ...
              return (
                <button
                  ...
                  type="button"
                >
                  <span>{taskNodeLabel(node.task, taskState, isChineseUi)}</span>
                </button>
              );
            })}
          </div>
        ...
        <p>{isChineseUi ? "生成任务后，这里会显示执行路径。" : "The execution path appears after tasks are planned."}</p>
      )}
    </section>
  );
}
...
function TaskList({
  ...
  return (
    <section className="fa-agent-team-simple-panel fa-agent-team-simple-task-list" aria-label={isChineseUi ? "任务" : "Tasks"}>
      <header className="fa-agent-team-simple-panel-header">
        <div>
          <span>{isChineseUi ? "任务" : "Tasks"}</span>
          <strong>{blockedTaskId ? (isChineseUi ? "先处理标红任务" : "Handle the red task first") : isChineseUi ? "任务清单" : "Task list"}</strong>
        </div>
      </header>
      {tasks.length ? (
          ...
          {tasks.map((task) => {
            ...
            return (
              <button
                ...
                type="button"
              >
                <span className="fa-agent-team-simple-task-index">{taskDependencyBadge(task, isChineseUi)}</span>
                <div className="fa-agent-team-simple-task-copy">
                  ...
                  <div className="fa-agent-team-task-status-line">
                    <span>{taskExecutionLabel(task, taskState, isChineseUi)}</span>
                    {taskHasOutput(task, viewModel.view) ? <span className="is-output-ready">{isChineseUi ? "有成果" : "Output ready"}</span> : null}
                    {task.attempt || task.max_attempts ? <span>{isChineseUi ? "尝试" : "Attempt"} {task.attempt ?? 0}/{task.max_attempts ?? "?"}</span> : null}
                    {task.heartbeat_at ? <span>{isChineseUi ? "有心跳" : "Heartbeat"}</span> : null}
                  </div>
                  <p>{isBlocked ? (isChineseUi ? "点这里查看原因和处理方式。" : "Click here to see the reason and next step.") : compactTaskGoal(task.goal)}</p>
                </div>
              </button>
            );
          })}
        ...
        <div className="fa-agent-team-simple-empty">
          ...
          <p>{isChineseUi ? "点上方主按钮生成任务。" : "Use the main button above to create tasks."}</p>
        </div>
      )}
    </section>
  );
...
function TaskDetail({
  ...
  actions: AgentTeamCockpitProps["actions"];
  selectedTaskState: AgentTeamCockpitViewModel["taskDisplayStates"][number] | null;
  session: AgentTeamSessionView;
  viewModel: AgentTeamCockpitViewModel;
}) {
  const { isChineseUi } = useShellUi();
  ...
  const taskArtifacts = task ? artifactsForTask(session.artifacts ?? [], task, taskOutputs) : [];
  const canRunSelectedTask = Boolean(task && selectedTaskState?.kind === "ready" && !viewModel.primaryAction.busy);
  const isBlocked = selectedTaskState?.kind === "failed" || selectedTaskState?.kind === "needs_attention";
  const retryTask = useRetryAgentTeamTask(task?.session_id ?? null);
  const canRetryTask = Boolean(task && isBlocked && !retryTask.isPending);
  const reasonInfo = task && isBlocked ? blockedReasonInfo(task, selectedTaskState, isChineseUi) : null;

  return (
    <details className={`fa-agent-team-simple-panel fa-agent-team-simple-detail ${isBlocked ? "is-blocked-detail" : ""}`.trim()} open={Boolean(task)}>
      <summary>{isBlocked ? (isChineseUi ? "处理卡住的任务" : "Fix blocked task") : isChineseUi ? "当前任务详情" : "Selected task details"}</summary>
      {task ? (
        <div className="fa-agent-team-simple-detail-body">
          <div className="fa-agent-team-simple-panel-header">
            <div>
              <span>{selectedTaskState?.label ?? statusLabel(task.status, isChineseUi)}</span>
              <strong>{taskTitle(task)}</strong>
            </div>
          </div>
          {reasonInfo ? (
            <section className="fa-agent-team-fix-box">
              ...
              <button
                ...
                onClick={() => task && retryTask.mutate({ taskId: task.task_id })}
                type="button"
              >
                {retryTask.isPending ? (isChineseUi ? "重试中..." : "Retrying...") : isChineseUi ? "重试这个任务" : "Retry this task"}
              </button>
            </section>
          ) : null}
          <section>
          ...
          <section>
            ...
            <p>{taskOutputSummary(task, taskOutputs, taskArtifacts, isChineseUi)}</p>
            <FieldList
              items={uniqueNonEmptyStrings([
                ...taskOutputs.map((output) => output.summary),
                ...taskArtifacts.map((artifact) => artifact.summary ?? artifact.title ?? artifact.uri),
        ...
        <p>{isChineseUi ? "生成任务后，这里会显示任务详情。" : "Task details will appear after planning."}</p>
      )}
    </details>
  );
}
  ...
  return (
    <section className="fa-agent-team-simple-panel fa-agent-team-outputs-panel">
      ...
      {outputItems.length ? (
        ...
        <p>{isChineseUi ? "任务完成后，成果会出现在这里。" : "Completed task outputs will appear here."}</p>
      )}
    </section>
  );
}
...
function FinalResultCard({
  ...
  return (
    <details className={`fa-agent-team-simple-panel fa-agent-team-simple-result is-${finalState.kind}`.trim()} open={finalState.hasBundle}>
      <summary>{isChineseUi ? "最终结果" : "Final result"}</summary>
      <div className="fa-agent-team-simple-detail-body">
        <header className="fa-agent-team-simple-panel-header">
          <div>
            <span>{isChineseUi ? "结果" : "Result"}</span>
          <small className="fa-sr-only">Decision Dock · Plan Review · Final Preview</small>
            <strong>{finalState.label}</strong>
          </div>
        </header>
        <p>{finalState.summary}</p>
        {!finalState.hasBundle ? <p>{isChineseUi ? "还没有最终汇总。你可以先查看上面的 Agent 产出。" : "No final summary yet. You can review the Agent outputs above."}</p> : null}
        {evidence.length ? (
          <div>
        ...
        <button
          ...
          type="button"
        >
          {actions.confirmResultPending ? (isChineseUi ? "确认中..." : "Approving...") : isChineseUi ? "确认完成" : "Approve final"}
        </button>
      </div>
    </details>
  );
...
function buildTaskGraph(tasks: AgentTeamTask[]): TaskGraph {
  ...
  function depthFor(task: AgentTeamTask): number {
    ...
    const dependencies = (task.dependencies ?? []).map((id) => taskById.get(id)).filter((item): item is AgentTeamTask => Boolean(item));
  ...
  const xGap = 116;
  const yGap = 72;
  for (const [depth, laneTasks] of [...lanes.entries()].sort(([left], [right]) => left - right)) {
  ...
  const edges = tasks.flatMap((task) =>
    (task.dependencies ?? []).filter((dependency) => nodeById.has(dependency)).map((dependency) => ({ from: dependency, to: task.task_id })),
  );
...
function taskHasOutput(task: AgentTeamTask, session: AgentTeamSessionView | null) {
...
function taskOutputItems(tasks: AgentTeamTask[], session: AgentTeamSessionView) {
  return tasks
    .map((task) => {
      const outputs = outputsForTask(session.outputs ?? [], task);
      const artifacts = artifactsForTask(session.artifacts ?? [], task, outputs);
    ...
    .filter((item): item is { summary: string; task: AgentTeamTask } => Boolean(item));
}
...
function taskNodeLabel(
  ...
  if (taskState?.kind === "running" || taskState?.kind === "queued") return isChineseUi ? "跑" : "Run";
...
function taskExecutionLabel(
  ...
  if (state === "needs_attention") return isChineseUi ? "需要处理" : "Needs attention";
  if (state === "waiting_dependency") return isChineseUi ? "等待前置任务" : "Waiting for dependency";
  if (runStatus && runStatus !== task.status) return isChineseUi ? `执行状态：${runStatus}` : `Run: ${runStatus}`;
...
function blockedReasonInfo(
  ...
  const rawReason = taskState?.lastError || task.last_error || taskState?.help || "";
  if (/delegated execution is disabled|automatic task execution is not enabled/i.test(rawReason)) {
    return {
  ...
  return {
    ...
    reason: rawReason || (isChineseUi ? "这个任务需要你先查看详情。" : "This task needs your review first."),
    nextStep: isChineseUi
...
function taskOutputSummary(
  ...
  const summary = latestOutput?.summary ?? latestArtifact?.summary ?? task?.verification_summary ?? task?.last_error;
  if (summary?.trim()) return summary.trim();
  if (!task) return isChineseUi ? "还没有任务产出。" : "No task output yet.";
  if (task.status === "done" || task.status === "completed") {
    return isChineseUi ? "任务已完成，等待汇总。" : "Task completed and ready for summary.";
  }
