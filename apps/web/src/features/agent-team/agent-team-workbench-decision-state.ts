...
export function isPlanReviewState({
	...
	return taskDisplayStates.every(
		(state) =>
			state.kind === "ready" ||
			state.kind === "pending" ||
			state.kind === "waiting_dependency",
	);
}
...
export function decisionDockStateForWorkbench({
	...
	return {
		...
		riskItems: finalPreviewState.riskItems.length
			? finalPreviewState.riskItems
			: riskItems,
		canGenerateResult,
...
export function primaryActionForWorkbench({
	...
	if (runningTasksCount || queuedTasksCount) {
		return {
			kind: "running",
			label:
				queuedTasksCount && !runningTasksCount
					? isChineseUi
						? "排队中..."
						: "Queued..."
					: isChineseUi
			...
			disabledReason: isChineseUi
				? "当前已有任务执行中或排队中"
				: "A task is already running or queued",
		};
	}
...
export function missionStageForWorkbench({
	...
	if (runningTasksCount || queuedTasksCount) {
		return {
			...
			help: isChineseUi
				? "任务正在执行或排队，等待产出回传。"
				: "Tasks are running or queued; wait for outputs to return.",
			tone: "neutral",
		};
	}
	...
	if (readyTasksCount) {
		return {
			...
			help: isChineseUi
				? "已有任务可运行，可以推进下一步。"
				: "At least one task is ready to run next.",
			tone: "neutral",
		};
	}
	...
	if (doneTasksCount === tasks.length && !activeBundle) {
		return {
			...
			help: isChineseUi
				? "所有任务已完成，可以生成最终结果。"
				: "All tasks are complete and ready for final synthesis.",
			tone: "success",
		};
	}
	...
	if (activeBundle) {
		...
		return {
			...
			help: isChineseUi
				? "最终结果已生成，可查看或重新生成。"
				: "A final result is available to review or regenerate.",
			tone: "success",
		};
	}
	...
	if (taskDisplayStates.some((state) => state.kind === "waiting_dependency")) {
		return {
			...
			help: isChineseUi
				? "部分任务需要等前置任务完成后再运行。"
				: "Some tasks are waiting for prerequisite work.",
			tone: "neutral",
		};
	}
	...
	return {
		...
		help: isChineseUi
			? "任务已创建，等待开始执行。"
			: "Tasks exist and are waiting to start.",
		tone: "neutral",
	};
}
...
export function blockedExplanationForWorkbench({
	...
	const failedCount = taskDisplayStates.filter(
	...
	const needsAttentionCount = taskDisplayStates.filter(
		(state) => state.kind === "needs_attention",
	).length;
	...
	const waitingDependencyCount = taskDisplayStates.filter(
...
export function nextStepForWorkbench({
	...
	if (!tasks.length) {
		return {
			label: isChineseUi
				? "先生成动态协作方案"
				: "Generate the dynamic collaboration plan first",
			help: isChineseUi
	...
	if (runningTasksCount) {
		return {
			label: isChineseUi
				? "Mission 正在运行，等待任务回传依据"
				: "Mission is running; wait for task evidence",
			help: isChineseUi
	...
	if (taskDisplayStates.some(isActionableTaskState)) {
		return {
			label: isChineseUi
				? "先处理需要注意的任务"
				: "Resolve tasks that need attention first",
			help:
				blockedExplanation ??
				(isChineseUi
	...
	if (readyTasksCount) {
		return {
			label: isChineseUi
				? "运行 Mission，自动推进可执行任务"
				: "Run the mission and advance ready tasks",
			help: isChineseUi
	...
	if (finalAnswerStatusForBundle(activeBundle) === "placeholder") {
		return {
			label: isChineseUi
				? "当前是模拟执行，需真实运行后再交付"
				: "This is simulated; run for real before delivery",
			help: isChineseUi
	...
	if (activeBundle?.recommended_next_action === "request_changes") {
		return {
			label: isChineseUi
				? "交付前先处理风险和修改项"
				: "Resolve risks and requested changes before delivery",
			help: isChineseUi
	...
	if (activeBundle) {
		return {
			label: isChineseUi
				? "查看最终结果并决定下一步"
				: "Review the final result and decide the next action",
			help: isChineseUi
	...
	if (evidenceItems.length || changedFiles.length) {
		return {
			label: isChineseUi
				? "已有产出后，生成最终结果"
				: "Generate a final result once outputs are ready",
			help: isChineseUi
	...
	return {
		label: isChineseUi
			? "检查任务依赖，必要时打开分支线程处理待办事项"
			: "Check dependencies and use branch threads for follow-up",
		help: isChineseUi
