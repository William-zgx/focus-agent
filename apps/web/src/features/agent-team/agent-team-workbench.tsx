...
export function AgentTeamWorkbench({
	sessionId,
}: {
	...
	const runMission = (taskIds?: string[]) =>
		runSession.mutate({
			...
			task_ids: taskIds?.length
				? taskIds
				: readyTasks.map((task) => task.task_id),
		});
	...
	const primaryActionBusy =
		...
		((primaryAction.kind === "generate_result" ||
			primaryAction.kind === "regenerate_result") &&
			mergeProposal.isPending);
	...
	const primaryActionDisabled =
		primaryActionBusy || Boolean(primaryAction.disabledReason);
	...
	const executionMode = executionModeForWorkbench(
		view.outputs ?? [],
		isChineseUi,
		view.run,
	);
	...
	return (
		<div className="fa-agent-team-layout fa-agent-team-workspace-shell fa-agent-team-stage fa-agent-team-guided-layout">
			<InlineMutationError
				...
				fallback={
					isChineseUi ? "取消 Mission 失败。" : "Failed to cancel mission."
				}
			/>
			<InlineMutationError
				...
				fallback={
					isChineseUi
						? "生成或调整计划失败。"
						: "Failed to generate or adjust the plan."
				}
			/>
			<InlineMutationError
			...
			{fallbackPlan ? (
				<div className="fa-agent-team-plan-banner">
					<strong>
						{isChineseUi
							? "模型规划不可用，已使用保守协作方案"
							: "Model planning is unavailable, so a conservative collaboration plan was used"}
					</strong>
					{session.planning_error ? (
						<span {...tooltipProps(session.planning_error)}>
							{session.planning_error}
						</span>
					) : null}
				</div>
			...
			<section className="fa-agent-team-cockpit-secondary-bar">
				...
				<Link
					...
					params={{
						conversationId: session.root_thread_id,
						threadId: session.root_thread_id,
					}}
				...
				<details className="fa-agent-team-more-menu">
					<summary className="fa-agent-team-cockpit-button is-secondary">
						{isChineseUi ? "更多" : "More"}
					</summary>
					<div className="fa-agent-team-more-menu-panel">
						<button
							disabled={planSession.isPending}
							onClick={tasks.length ? confirmAndReplanMission : generatePlan}
							type="button"
						>
							{tasks.length
								? isChineseUi
						...
						<button
							disabled={planSession.isPending}
							onClick={confirmAndAdjustMissionBreakdown}
							type="button"
						>
							{isChineseUi ? "调整拆解" : "Adjust breakdown"}
						</button>
						<button
							disabled={mergeProposal.isPending || !canGenerateResult}
							onClick={generateResult}
							type="button"
						>
							{isChineseUi ? "生成最终结果" : "Generate final result"}
						</button>
						<hr className="fa-agent-team-menu-divider" />
						<button
							className="is-danger"
...
function executionModeForWorkbench(
	...
	if (normalized === "inline" || normalized === "observe")
