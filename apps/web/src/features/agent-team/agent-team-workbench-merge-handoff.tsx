  ...
  return (
    <section className="fa-agent-team-panel">
      <div className="fa-agent-team-panel-header">
        <div>
          <span>{isChineseUi ? "结论、依据、需要注意" : "Conclusion, evidence, and risks"}</span>
        ...
        {bundle ? <FinalAnswerStatusPill bundle={bundle} isChineseUi={isChineseUi} /> : null}
      </div>
      {bundle ? (
        <div className="fa-agent-team-detail">
          <section>
            <h3>{isChineseUi ? "Agent Team 最终答案" : "Agent Team final answer"}</h3>
            <p className="fa-agent-team-final-answer-text">{finalAnswerText(bundle, isChineseUi)}</p>
          </section>
          ...
          <section>
            ...
            <FieldList items={resultEvidenceItems(bundle, evidenceItems, isChineseUi)} />
          </section>
          <section>
            <h3>{isChineseUi ? "需要注意" : "Needs attention"}</h3>
            <FieldList items={resultRiskItems(bundle, riskItems, isChineseUi)} />
          </section>
        </div>
      ) : (
...
export function AdvancedDetailsPanel({
  ...
  const advancedChangedFiles = bundle?.changed_files?.length ? bundle.changed_files : changedFiles;
  const artifactItems = artifacts.map((artifact) => artifact.summary ?? artifact.title ?? artifact.uri ?? artifact.artifact_id);
  const sourceOutputItems = bundle?.source_output_ids?.length ? bundle.source_output_ids : outputs.map((output) => output.output_id);
  ...
  return (
    <section className="fa-agent-team-panel">
      <div className="fa-agent-team-panel-header">
        <div>
          <span>{isChineseUi ? "Planning metadata、DAG、raw evidence" : "Planning metadata, DAG, raw evidence"}</span>
      ...
      <div className="fa-agent-team-detail">
        ...
        <section>
          ...
          <FieldList items={bundle?.risk_items?.length ? bundle.risk_items : riskItems} />
        </section>
        <section>
    ...
    <section className="fa-agent-team-panel fa-agent-team-merge-card">
      ...
      {activeBundle ? (
        <div className="fa-agent-team-merge-grid">
          <p className="fa-agent-team-final-answer-text">{finalAnswerText(activeBundle, isChineseUi)}</p>
          <div>
        ...
        <EmptyList>{isChineseUi ? "还没有生成最终结果。" : "No final result generated yet."}</EmptyList>
      )}
      {error ? <div className="fa-inline-notice is-danger">{error.message}</div> : null}
      {!hideAction ? (
        <button
...
function FinalAnswerStatusPill({ bundle, isChineseUi }: { bundle: AgentTeamMergeBundle; isChineseUi: boolean }) {
      ...
      status === "ready"
        ...
        : status === "placeholder" || status === "blocked"
          ...
          : status === "error"
            ? "danger"
            : "neutral";
    return <span className={`fa-agent-team-pill is-${tone}`}>{labels[status] ?? status}</span>;
  }
  return <RecommendedActionPill action={bundle.recommended_next_action ?? ""} isChineseUi={isChineseUi} />;
}

function RecommendedActionPill({ action, isChineseUi }: { action: string; isChineseUi: boolean }) {
  const labels: Record<string, string> = isChineseUi
    ? {
        merge: "可交付",
  ...
  const tone = action === "merge" ? "success" : action === "request_changes" ? "warning" : "neutral";

  return <span className={`fa-agent-team-pill is-${tone}`}>{labels[action] ?? action.replaceAll("_", " ")}</span>;
}
...
function finalResultActionLabel({
  ...
  if (isGenerating) return isChineseUi ? "正在生成最终结果..." : "Generating final result...";
  if (!canGenerate) return isChineseUi ? "等待任务产出" : "Waiting for task outputs";
  if (hasBundle) return isChineseUi ? "重新生成最终结果" : "Regenerate final result";
  return isChineseUi ? "生成最终结果" : "Generate final result";
}

function resultEvidenceItems(bundle: AgentTeamMergeBundle, evidenceItems: string[], isChineseUi: boolean) {
...
function finalResultSummary(bundle: AgentTeamMergeBundle | null, isChineseUi: boolean) {
...
function finalAnswerWarnings(bundle: AgentTeamMergeBundle, isChineseUi: boolean) {
  if (isPlaceholderBundle(bundle)) {
    return [
      isChineseUi
...
function resultRiskItems(bundle: AgentTeamMergeBundle, riskItems: string[], isChineseUi: boolean) {
  if (isPlaceholderBundle(bundle)) {
    return [
      isChineseUi
  ...
  return items.length ? items : [isChineseUi ? "暂无需要处理的事项。" : "No items need attention."];
}
...
function branchCluesFromTasks(tasks: AgentTeamTask[]) {
  return uniqueStrings(
    tasks.flatMap((task) => {
      const clues = [];
      if (task.branch_id) clues.push(`${task.title ?? task.task_id}: branch ${task.branch_id}`);
      if (task.child_thread_id) clues.push(`${task.title ?? task.task_id}: thread ${task.child_thread_id}`);
      if (task.dependencies?.length) clues.push(`${task.title ?? task.task_id}: depends on ${task.dependencies.join(", ")}`);
      return clues;
    }),
  );
}
...
function dagItems(dag: Record<string, unknown> | null | undefined) {
  if (!dag) return [];
  return Object.entries(dag).map(([key, value]) => `${key}: ${formatUnknown(value)}`);
}

function planningMetadataItems(metadata: AgentTeamPlanningMetadata | Record<string, unknown> | null | undefined) {
  if (!metadata) return [];
...
function limitUserFacingItems(items: string[], isChineseUi: boolean, limit = 6) {
  ...
  return [
    ...items.slice(0, limit),
    isChineseUi ? `另有 ${items.length - limit} 条可在高级详情查看。` : `${items.length - limit} more in advanced details.`,
  ];
}
