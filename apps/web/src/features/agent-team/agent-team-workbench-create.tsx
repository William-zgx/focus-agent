...
export function CreateSessionPanel() {
	const dagEdgeGradientId = useId().replace(/:/g, "");
	const { isChineseUi } = useShellUi();
		...
		<div className="fa-agent-team-layout fa-agent-team-workspace-shell fa-agent-team-studio is-create">
			<section className="fa-agent-team-studio-header">
				...
				>
					...
					<path
						className="edge"
						d="M45 41 C64 18 75 18 94 22"
						stroke={`url(#${dagEdgeGradientId})`}
					/>
					<path
						className="edge"
						d="M45 41 C64 64 75 64 94 60"
						stroke={`url(#${dagEdgeGradientId})`}
					/>
					<path
						className="edge"
						d="M122 22 C137 18 145 22 158 31"
						stroke={`url(#${dagEdgeGradientId})`}
					/>
					<path
						className="edge"
						d="M122 60 C137 64 145 60 158 51"
						stroke={`url(#${dagEdgeGradientId})`}
					/>
					<path
						className="edge edge-strong"
						d="M184 41 C197 41 204 41 216 41"
						stroke={`url(#${dagEdgeGradientId})`}
					/>
