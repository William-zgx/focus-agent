...
export function runStatusText(task: AgentTeamTask) {
  if (typeof task.run_status === "string" && task.run_status.trim()) return task.run_status;
  if (isRecord(task.run_status)) {
    return String(task.run_status.status ?? task.run_status.state ?? task.execution_status ?? task.status);
  }
  return String(task.execution_status ?? task.status);
}

export function runStatusDetails(task: AgentTeamTask): string[] {
  if (!isRecord(task.run_status)) return uniqueNonEmptyStrings([task.execution_status ?? null]);
  return uniqueNonEmptyStrings([
    typeof task.run_status.run_id === "string" ? `run_id: ${task.run_status.run_id}` : null,
    typeof task.run_status.message === "string" ? task.run_status.message : null,
    typeof task.run_status.error === "string" ? task.run_status.error : null,
    typeof task.run_status.started_at === "string" ? `started_at: ${task.run_status.started_at}` : null,
    typeof task.run_status.finished_at === "string" ? `finished_at: ${task.run_status.finished_at}` : null,
  ]);
}
...
export function isTaskDone(task: AgentTeamTask) {
  return task.status === "done" || task.status === "completed" || runStatusText(task) === "done";
}

export function isTaskReady(task: AgentTeamTask, tasks: AgentTeamTask[]) {
  const legacyPrimedTask =
    task.status === "running" && !task.run_status && !task.execution_status && !task.agent_run_id;
  if (task.status !== "pending" && task.status !== "ready" && !legacyPrimedTask) return false;
  const doneTaskIds = new Set(tasks.filter(isTaskDone).map((item) => item.task_id));
  return (task.dependencies ?? []).every((dependency) => doneTaskIds.has(dependency));
}
...
export function deriveTaskDisplayState(
  ...
  if (isTaskDone(task)) {
    return {
      ...
      help: isChineseUi ? "任务已完成，产出可用于最终结果。" : "The task is complete and ready for final synthesis.",
      tone: "success",
  ...
  if (task.status === "failed") {
    return {
      ...
      help: lastError ?? (isChineseUi ? "任务执行失败，请查看详情后处理。" : "The task failed. Review details before continuing."),
      tone: "danger",
      incompleteDependencies,
      lastError,
    };
  ...
  if (isTaskReady(task, tasks)) {
    return {
      ...
      help: isChineseUi ? "前置任务已完成，可以运行此任务。" : "Dependencies are complete; this task can run.",
      tone: "neutral",
      incompleteDependencies,
      lastError,
    };
  ...
  return {
    ...
    help: isChineseUi ? "任务已创建，尚未开始执行。" : "The task exists and has not started yet.",
    tone: "neutral",
    incompleteDependencies,
    lastError,
  };
...
function taskIncompleteDependencies(task: AgentTeamTask, tasks: AgentTeamTask[]) {
  const doneTaskIds = new Set(tasks.filter(isTaskDone).map((item) => item.task_id));
  return uniqueNonEmptyStrings((task.dependencies ?? []).filter((dependency) => !doneTaskIds.has(dependency)));
}
...
export function normalizeSessionView(data: AgentTeamSession | AgentTeamSessionView | undefined): AgentTeamSessionView | null {
  ...
  if ("session" in data) {
    ...
    return {
      ...
      merge_bundle: data.merge_bundle ?? data.merge_suggestion ?? data.session.latest_merge_bundle ?? null,
      planning,
  ...
  return {
    ...
    outputs: Array.isArray(dataRecord.outputs) ? (dataRecord.outputs as AgentTeamSessionView["outputs"]) : [],
...
function sessionWithPlanningMetadata(
  ...
  return {
    ...
    planning_rationale: session.planning_rationale ?? planning.rationale ?? null,
    planner_model_id: session.planner_model_id ?? planning.planner_model_id ?? null,
    plan_generated_at: session.plan_generated_at ?? planning.generated_at ?? null,
...
export function runReadyTasksActionLabel({
  ...
  return isChineseUi ? `运行 Mission (${readyCount})` : `Run Mission (${readyCount})`;
}
...
export function planningSourceLabel(source: string | null | undefined, isChineseUi: boolean) {
...
export function isFallbackPlan(session: AgentTeamSession | null, tasks: AgentTeamTask[]) {
  const sessionSource = session?.planning_source?.toLowerCase() ?? "";
  if (session?.planning_error) return true;
  if (["fallback", "fallback_heuristic", "conservative", "default"].includes(sessionSource)) {
    return true;
  }
