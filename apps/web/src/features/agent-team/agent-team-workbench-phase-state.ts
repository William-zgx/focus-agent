...
export type AgentTeamWorkbenchPhaseKey =
	| "plan"
	| "execute"
	| "verify"
	| "synthesize";
...
export type AgentTeamWorkbenchPhaseMapStatus =
	| "empty"
	| "waiting"
	| "ready"
	| "running"
...
export interface AgentTeamWorkbenchPhaseMapItem
	extends AgentTeamWorkbenchPhaseGroup {
	taskIds: string[];
	...
	const groups: AgentTeamWorkbenchPhaseGroup[] = [
		{
			...
			help: isChineseUi
				? "识别目标、交付物和依赖边界。"
				: "Identify the goal, deliverables, and dependency boundaries.",
			tasks: [],
			completedCount: 0,
			blockedCount: 0,
			readyCount: 0,
		},
		{
			...
			help: isChineseUi
				? "产出可交付改动、材料或结论。"
				: "Produce deliverable changes, artifacts, or findings.",
			tasks: [],
			completedCount: 0,
			blockedCount: 0,
			readyCount: 0,
		},
		{
			...
			help: isChineseUi
				? "补齐测试、审查和验收证据。"
				: "Collect tests, review notes, and acceptance evidence.",
			tasks: [],
			completedCount: 0,
			blockedCount: 0,
			readyCount: 0,
		},
		{
			...
			help: isChineseUi
				? "收束最终回答、风险和开放问题。"
				: "Synthesize final answer, risks, and open questions.",
			tasks: [],
			completedCount: 0,
			blockedCount: 0,
			readyCount: 0,
...
export function buildPhaseMapItems({
	...
	return phaseGroups.map((group) => {
		...
		const hasRecommendedTask = Boolean(
			recommendedTaskId && taskIds.includes(recommendedTaskId),
		);
		...
		const isComplete = Boolean(
			group.tasks.length && group.completedCount >= group.tasks.length,
		);
		return {
			...group,
			taskIds,
...
function phaseMapStatusForGroup(
	group: AgentTeamWorkbenchPhaseGroup,
): AgentTeamWorkbenchPhaseMapStatus {
	if (!group.tasks.length) return "empty";
	if (group.blockedCount) return "attention";
	if (group.tasks.some((task) => isTaskRunning(task) || isTaskQueued(task)))
		return "running";
...
function phaseKeyForTask(task: AgentTeamTask): AgentTeamWorkbenchPhaseKey {
	const descriptor = [task.role, task.task_type, task.title, task.goal]
		.join(" ")
		.toLowerCase();
	if (
		/(plan|planner|architect|research|design|拆解|规划|方案)/.test(descriptor)
	)
		return "plan";
	if (
		/(test|verify|verification|review|qa|risk|审查|验证|测试|风险)/.test(
			descriptor,
		)
	)
		return "verify";
	if (
		/(synth|merge|writer|summary|final|handoff|汇总|收束|交付|总结)/.test(
			descriptor,
		)
	)
		return "synthesize";
