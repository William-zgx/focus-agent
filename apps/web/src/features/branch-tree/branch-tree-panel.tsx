...
export function BranchTreePanel() {
	...
	const selectedThreadId =
		selectedContextThreadId || selectedNode?.thread_id || "";
	const graph = useMemo(
		() => buildGraph(data?.root, selectedThreadId),
		[data?.root, selectedThreadId],
	);
	...
	const createBranchTargetNode =
		...
		data?.archived_branches?.find(
			(item) => item.thread_id === createBranchTargetThreadId,
		) ??
		null;
	const isMergedCreateTarget =
		createBranchTargetNode?.branch_status === "merged";
	const {
		...
		<div className="fa-branch-panel">
			<section className="fa-tree-card">
				<BranchTreeGraphToolbar
					...
					onCreateBranch={() =>
						void createBranchFromTarget(
							createBranchTargetThreadId,
				...
				<div className="fa-tree-panel-body">
					...
					<div className="fa-tree-legend">
						<span className="fa-tree-legend-item is-role-main">
							{isChineseUi ? "主线时间轴" : "Main timeline"}
						</span>
						<span className="fa-tree-legend-item is-role-explore">
							{isChineseUi ? "探索" : "Explore"}
						</span>
						<span className="fa-tree-legend-item is-role-deep-dive">
							{isChineseUi ? "深挖" : "Deep dive"}
						</span>
						<span className="fa-tree-legend-item is-role-execute">
							{isChineseUi ? "执行" : "Execute"}
						</span>
						<span className="fa-tree-legend-item is-role-verify">
							{isChineseUi ? "验证" : "Verify"}
						</span>
						<span className="fa-tree-legend-item is-role-writeup">
							{isChineseUi ? "写作" : "Writeup"}
						</span>
					</div>
					...
					<div className="fa-tree-canvas-shell">
						<BranchTreeCanvasTools
							...
							onCenterSelectedNode={() =>
								centerSelectedNode(branchZoomRef.current, "smooth")
							}
			...
			<ArchivedConversationsSection
				...
				isChineseUi={isChineseUi}
				isWorking={isWorking}
				onOpenConversation={(rootThreadId) =>
					void openConversation(rootThreadId)
				}
