...
export function clampBranchZoom(value: number) {
	return Math.min(
		BRANCH_ZOOM_MAX,
		Math.max(BRANCH_ZOOM_MIN, Number(value.toFixed(2))),
	);
}
...
export function useBranchTreeViewport({
	...
	useEffect(() => {
		...
		function handleWheel(event: WheelEvent) {
			...
			const nextZoom =
				branchZoomRef.current *
				Math.exp(-event.deltaY * BRANCH_ZOOM_WHEEL_SENSITIVITY);
