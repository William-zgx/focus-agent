...
export function useBranchTreeActions({
  ...
  const { archiveBranch, activateBranch, prepareMergeProposal, renameBranch } = useBranchActions({
    rootThreadId,
  ...
  const detailActionViewModel = useMemo(() => {
    ...
    return {
      ...
      detailHasPreparedConclusion,
      detailNodeStatusTone: detailNode ? statusAccentTone(detailNode.branch_status) : "",
    };
  }, [detailNode, getMergeProposalError, isChineseUi, isMergeProposalPreparing]);
