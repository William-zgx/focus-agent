...
export function BranchNodeDetailOverlay({
	...
	const metaRows = [
		...
		[
			roleLabelText(isChineseUi),
			roleLabel(detailNode.branch_role, isChineseUi),
		],
		[
			statusLabel(isChineseUi),
		...
		>
			...
			>
					...
					{detailNode.branch_id ? (
						<button
							...
							{...tooltipProps(
								isChineseUi
									? "双击重命名这个分支"
									: "Double-click to rename this branch",
							)}
							disabled={isWorking}
							onDoubleClick={() => onStartRename(detailNode)}
							type="button"
						>
							{detailNode.branch_name}
						</button>
					) : (
				...
				<div className="fa-branch-node-badges">
					...
					<span
						className={`fa-branch-node-badge ${detailNodeStatusTone}`.trim()}
					>
						{branchStatusLabel(detailNode.branch_status, isChineseUi)}
					</span>
					<span className="fa-branch-node-badge">
				...
				<div className="fa-branch-node-actions">
					...
					renameBranchTarget?.thread_id !== detailNode.thread_id ? (
						<button
							className="fa-branch-inline-action"
							{...tooltipProps(
								isChineseUi ? "重命名这个分支" : "Rename this branch",
							)}
							disabled={isWorking}
					...
					{detailCanReviewConclusion ? (
						<button
							...
							onClick={() =>
								detailHasPreparedConclusion
									? onViewMergeReview(detailNode)
									: onPrepareProposal(detailNode)
							}
							type="button"
						>
							{detailConclusionActionLabel}
					...
					{detailCanReviewConclusion && detailHasPreparedConclusion ? (
						<button
							...
							{...tooltipProps(
								isChineseUi
									? "重新生成这个分支的结论"
									: "Regenerate conclusion for this branch",
							)}
							disabled={isWorking || detailConclusionPreparing}
