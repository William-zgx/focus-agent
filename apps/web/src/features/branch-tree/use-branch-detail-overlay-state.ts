...
export function useBranchDetailOverlayState({
  ...
  const getBranchDetailStyle = useCallback((anchor: HTMLElement): CSSProperties => {
  ...
  const showBranchDetail = useCallback(
    (node: BranchTreeNode, anchorElement: HTMLElement | null) => {
      ...
      setDetailThreadId((current) => (current === node.thread_id ? current : node.thread_id));
      if (!isSameAnchor) {
        updateBranchDetailPosition();
      }
    },
    [clearBranchDetailHideTimer, getBranchDetailStyle, updateBranchDetailPosition],
  );
  ...
  useEffect(() => () => clearBranchDetailHideTimer(), [clearBranchDetailHideTimer]);
