    ...
    <section className="fa-agent-role-grid">
      <div className="fa-observability-list-panel fa-agent-role-panel">
        <PanelHeader
          ...
          meta={delegationTrajectoryAvailable ? `${recentDelegationRuns.length} runs` : "not available"}
          title={isChineseUi ? "多 Agent 执行轨迹" : "Multi-Agent Execution"}
        />
        <TrajectoryDetailsList
          empty={isChineseUi ? "还没有 agent_delegation_plan 记录。" : "No agent_delegation_plan records yet."}
      ...
      <div className="fa-observability-detail-panel fa-agent-role-panel">
        <PanelHeader
          ...
          title={isChineseUi ? "成本 / 质量 / 延迟路由" : "Cost / Quality / Latency"}
        />
        <KeyValueList
          rows={Object.entries(modelRouterRoleModels ?? {}).map(([role, model]) => ({
        ...
        <TrajectoryDetailsList
          ...
          getSummary={(item) => ({
            ...
            value: String(item.effective_model ?? item.recommended_model ?? "-"),
          })}
  ...
  return (
    <section className="fa-observability-detail-block fa-agent-role-trajectory">
      <PanelHeader
        ...
        meta={criticTrajectoryAvailable ? `${criticVerdictCount} verdicts` : "not available"}
        title={isChineseUi ? "产物验收结果" : "Artifact Verdicts"}
      />
      <TrajectoryDetailsList
        empty={isChineseUi ? "还没有 critic_gate_result trajectory 记录。" : "No critic gate verdict records yet."}
    ...
    <section className="fa-agent-role-grid">
      <div className="fa-observability-list-panel fa-agent-role-panel">
        <PanelHeader
          ...
          meta={selfRepairTrajectoryAvailable ? `${recentFailures.length} failures` : "not available"}
          title={isChineseUi ? "失败归因与候选样本" : "Failure Triage"}
        />
        <TrajectoryDetailsList
          empty={isChineseUi ? "还没有 agent failure 记录。" : "No agent failure records yet."}
      ...
      <div className="fa-observability-detail-panel fa-agent-role-panel">
        <PanelHeader
          ...
          meta={reviewQueueTrajectoryAvailable ? `${reviewQueueItems.length} items` : "not available"}
          title={isChineseUi ? "人工干预队列" : "Human Review Queue"}
        />
        <TrajectoryDetailsList
          empty={isChineseUi ? "还没有待人工确认的治理项。" : "No pending governance review items."}
  ...
  return (
    <div className="fa-observability-detail-panel fa-agent-role-panel">
      <PanelHeader
        ...
        meta={contextArtifactsTrajectoryAvailable ? `${recentContextArtifacts.length} refs` : "not available"}
        title={isChineseUi ? "Artifact 化证据" : "Artifactized Evidence"}
      />
      <TrajectoryDetailsList
        empty={isChineseUi ? "还没有 context artifact trajectory 记录。" : "No context artifact trajectory records yet."}
  ...
  return (
    <section className="fa-observability-detail-block fa-agent-role-trajectory">
      <PanelHeader
        ...
        meta={capabilitiesIsLoading ? "loading" : `${capabilityItems.length} tools`}
        title={isChineseUi ? "工具能力注册表" : "Tool Capability Registry"}
      />
      {capabilitiesError ? (
        <InlineDangerNotice>{errorMessage(capabilitiesError, "Failed to load capabilities")}</InlineDangerNotice>
      ) : null}
      <KeyValueList
      ...
      {!capabilityItems.length ? (
        <EmptyState>{isChineseUi ? "当前没有可展示的工具能力。" : "No tool capabilities to display."}</EmptyState>
      ) : null}
    </section>
  );
}
  ...
  return (
    <section className="fa-observability-detail-block fa-agent-role-trajectory">
      <PanelHeader
        ...
        meta={contextDecisionsTrajectoryAvailable ? `${contextDecisionCount} records` : "not available"}
        title={isChineseUi ? "最近上下文预算记录" : "Recent Context Budget Records"}
      />
      <TrajectoryDetailsList
        empty={isChineseUi ? "还没有 context_budget_decision trajectory 记录。" : "No context budget records yet."}
...
export function ToolRouterTrajectoryPanel({
  ...
  return (
    <section className="fa-observability-detail-block fa-agent-role-trajectory">
      <PanelHeader
        eyebrow={isChineseUi ? "Tool Router Trajectory" : "Tool Router Trajectory"}
        meta={toolRouteTrajectoryAvailable ? `${toolRouteDecisionCount} records` : "not available"}
        title={isChineseUi ? "最近工具路由记录" : "Recent Tool Route Records"}
      />
      <TrajectoryDetailsList
        empty={isChineseUi ? "还没有 tool_route_plan trajectory 记录。" : "No tool_route_plan trajectory records yet."}
  ...
  return (
    <section className="fa-observability-detail-block fa-agent-role-trajectory">
      <PanelHeader
        ...
        meta={decisionsTrajectoryAvailable ? `${decisionCount} records` : "not available"}
        title={isChineseUi ? "最近决策记录" : "Recent Decision Records"}
      />
      {decisionsTrajectoryError ? <InlineDangerNotice>{decisionsTrajectoryError}</InlineDangerNotice> : null}
      <RoleTrajectoryList
        empty={
          isChineseUi
