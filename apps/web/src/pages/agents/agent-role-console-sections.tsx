...
type TrajectoryDetailsListProps<T> = {
  ...
  getSummary: (item: T, index: number) => {
    label: ReactNode;
    value: ReactNode;
  };
  empty?: ReactNode;
...
export function KeyValueList({ rows }: KeyValueListProps) {
  return (
    <div className="fa-agent-role-model-list">
      {rows.map((row, index) => (
        <div className="fa-agent-role-model-row" key={String(row.label ?? index)}>
          <span>{row.label}</span>
...
export function TrajectoryDetailsList<T>({
  ...
  const content = (
    <>
      {visibleItems.map((item, index) => {
        ...
        return (
          <details className="fa-agent-role-trajectory-row" key={getKey(item, index)}>
            <summary>
  ...
  return wrap ? <div className="fa-agent-role-trajectory-list">{content}</div> : content;
}

export function RoleDecisionCards({ decisions }: { decisions: DecisionCard[] }) {
  return (
...
export function ToolRouteDecisionCards({ decisions }: { decisions: DecisionCard[] }) {
  return (
    <div className="fa-agent-role-decision-list">
      {decisions.map((decision, index) => (
    ...
    <section className="fa-observability-hero fa-agent-role-hero">
      ...
      <div className="fa-observability-hero-grid fa-agent-role-policy-grid">
        ...
        <div className="fa-observability-stat-card">
          ...
          <p>{artifactizeLongObservations ? "artifact refs on" : "preview safe"}</p>
        </div>
        <div className="fa-observability-stat-card">
