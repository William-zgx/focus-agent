...
export function RoleModelMappingPanel({
  ...
  return (
    <div className="fa-observability-list-panel fa-agent-role-panel">
      ...
      />
      {policyError ? (
        <InlineDangerNotice>{errorMessage(policyError, "Failed to load role policy")}</InlineDangerNotice>
      ) : null}
  ...
  return (
    <div className="fa-observability-list-panel fa-agent-role-panel">
      <PanelHeader
        ...
        meta={memoryPolicyIsLoading ? "loading" : memoryPolicyData?.conflict_strategy ?? "needs_review"}
        title={isChineseUi ? "分支语义保护" : "Branch Semantic Guard"}
      />
      {memoryPolicyError ? (
        <InlineDangerNotice>
          {errorMessage(memoryPolicyError, "Failed to load memory curator policy")}
        </InlineDangerNotice>
      ) : null}
      <KeyValueList
        rows={[
  ...
  return (
    <div className="fa-observability-list-panel fa-agent-role-panel">
      <PanelHeader
        ...
        meta={taskLedgerTrajectoryAvailable ? `${recentTaskLedgerRuns.length} tasks` : "not available"}
        title={isChineseUi ? "任务账本与 DAG" : "Task DAG"}
      />
      {taskLedgerPolicyError ? (
        <InlineDangerNotice>
          {errorMessage(taskLedgerPolicyError, "Failed to load task ledger policy")}
        </InlineDangerNotice>
      ) : null}
      <KeyValueList
        rows={[
          ...
          },
          {
            ...
            value: String(taskLedgerPolicyData?.artifact_synthesis_enabled ?? false),
          },
          {
  ...
  return (
    <div className="fa-observability-detail-panel fa-agent-role-panel">
      <PanelHeader
        ...
        meta={delegatedArtifactsTrajectoryAvailable ? `${recentDelegatedArtifacts.length} artifacts` : "not available"}
...
export function ContextPolicyPanel({
  ...
  return (
    <div className="fa-observability-list-panel fa-agent-role-panel">
      ...
      />
      {contextPolicyError ? (
        <InlineDangerNotice>{errorMessage(contextPolicyError, "Failed to load context policy")}</InlineDangerNotice>
      ) : null}
      <KeyValueList
        rows={[
          {
