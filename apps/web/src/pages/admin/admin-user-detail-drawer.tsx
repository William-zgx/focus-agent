...
const USER_DETAIL_TABS: UserDetailTab[] = ["profile", "access", "security", "audit"];
      ...
      <div className="fa-admin-drawer-sticky">
        <header className="fa-admin-drawer-header">
          <div>
            ...
            <p>{isChineseUi ? "资料、权限、安全和近期审计集中在这里处理。" : "Profile, access, security, and recent audit activity in one place."}</p>
          </div>
          <button className="fa-admin-icon-button" type="button" onClick={onClose} aria-label={isChineseUi ? "关闭详情" : "Close detail"}>
            x
          </button>
        </header>
        ...
        <div className="fa-admin-drawer-meta">
          ...
          {user ? <span className={`fa-observability-pill is-${statusTone(user.status)}`}>{user.status}</span> : null}
        </div>

        {userQuery.error ? <AdminErrorMessage error={userQuery.error} fallback="Failed to load user." /> : null}

        <nav className="fa-admin-drawer-tabs" aria-label={isChineseUi ? "用户详情分区" : "User detail sections"}>
