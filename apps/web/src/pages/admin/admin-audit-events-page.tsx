...
export function AdminAuditEventsPage() {
	...
	const [actorFilter, setActorFilter] = useState(() =>
		readAdminSearchParam("actor"),
	);
	const [resourceTypeFilter, setResourceTypeFilter] = useState(() =>
		readAdminSearchParam("resource_type"),
	);
	const [resourceIdFilter, setResourceIdFilter] = useState(() =>
		readAdminSearchParam("resource_id"),
	);
	const [decisionFilter, setDecisionFilter] = useState(() =>
		readAdminSearchParam("decision"),
	);
	const [selectedEventId, setSelectedEventId] = useState(() =>
		readAdminSearchParam("event"),
	);
	...
	const highRiskCount = events.filter((event) =>
		...
		].some((prefix) => event.action.includes(prefix)),
	).length;
	const deniedCount = events.filter(
		(event) => decisionTone(event.decision) === "danger",
	).length;
			...
			<section className="fa-admin-panel fa-admin-primary-panel">
				...
				{auditQuery.error ? (
					<AdminErrorMessage
						...
						fallback="Failed to load audit events."
					/>
				) : null}
...
function ResourceCell({ event }: { event: FocusAgentAuditEvent }) {
	const isUserResource =
		event.resource_type === "user" && Boolean(event.resource_id);
		...
		<div className="fa-admin-drawer-shell">
			<header className="fa-admin-drawer-header">
				...
				<button
					...
					type="button"
					onClick={onClose}
					aria-label={isChineseUi ? "关闭审计详情" : "Close audit detail"}
				>
					x
				</button>
			</header>
			...
			<div className="fa-admin-drawer-body">
				...
				<section className="fa-admin-panel">
					...
					/>
					<pre className="fa-admin-json-block">
