...
import { useTrajectoryStats } from "@/features/trajectory-observability/use-trajectory-stats";
...
export function TrajectoryPage() {
  ...
  const filters = useMemo<FocusAgentTrajectoryListRequest>(
    () => ({
      ...
      min_latency_ms: minLatency.trim() ? Number(minLatency) : undefined,
      fallback_used: fallbackOnly || undefined,
      has_error: hasErrorOnly || undefined,
      limit: 80,
    }),
    [fallbackOnly, hasErrorOnly, minLatency, modelFilter, statusFilter, threadFilter, toolFilter],
  );
  const deferredFilters = useDeferredValue(filters);
  const { data: listData, isLoading: isListLoading, error: listError } = useTrajectoryList(deferredFilters);
  const { data: statsData, isLoading: isStatsLoading, error: statsError } = useTrajectoryStats({
    ...deferredFilters,
  });
  ...
  const filterChips = useMemo(
    ...
    [fallbackOnly, hasErrorOnly, minLatency, modelFilter, sortMode, statusFilter, threadFilter, toolFilter],
  );
              ...
              <div className="fa-observability-filter-shell">
                <div className="fa-observability-filters is-compact">
                  ...
                  <label className="fa-observability-filter">
                    ...
                    <input value={minLatency} onChange={(event) => setMinLatency(event.target.value)} inputMode="numeric" placeholder="500" />
                  </label>
                  <label className="fa-observability-filter">
