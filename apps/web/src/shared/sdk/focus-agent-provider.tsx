...
export function FocusAgentProvider({ children }: PropsWithChildren) {
	...
	const [authHint, setAuthHint] = useState<
		"demo_token_disabled" | "manual_token" | null
	>(null);
	...
	const resolvePrincipalFromAuthResponse = useCallback(
		...
		): Promise<FocusAgentPrincipalResponse> => {
			...
			if (authAttemptRef.current === authAttemptId) {
				...
				setAuthError(
					isUnauthorized(error)
						? null
						: authErrorMessage(error, "Failed to refresh the current session."),
				);
	...
	useEffect(() => {
		...
		void bootstrap().catch((error: unknown) => {
			if (shouldApplyBootstrapResult()) {
				...
				setPrincipal(null);
				setAuthError(
					authErrorMessage(error, "Failed to bootstrap Focus Agent auth."),
				);
				setAuthHint("manual_token");
				setReady(true);
			}
