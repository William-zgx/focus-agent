...
const defaultMaxImportant = 0;

function parseArgs(argv) {
	const config = {
		max: defaultMaxImportant,
		roots: defaultRoots,
		showAll: false,
	};
...
function printHelp() {
	console.log(`Usage: node ./scripts/check-no-important.mjs [options]

Checks CSS governance for !important usage.

Options:
  --max <count>       Maximum allowed !important occurrences. Default: ${defaultMaxImportant}
...
function findMatches(filePath) {
	...
	source.split(/\r?\n/).forEach((line, lineIndex) => {
		...
		let match = importantPattern.exec(line);
		while (match) {
			matches.push({
			...
			match = importantPattern.exec(line);
		}
	});
...
function main() {
	...
	console.log("CSS governance: no !important");
	...
	if (overBudget) {
		console.error(
			`\nFailed: !important occurrences (${matches.length}) exceed budget (${config.max}).`,
		);
		process.exitCode = 1;
	} else {
		console.log("\nPassed: !important usage is within budget.");
	}
}
